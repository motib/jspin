...
\mbox{}\\\large{Version 1.6}}       % Change version number
...
\begin{center}
Copyright \copyright{} 2009-11 by Mordechai (Moti) Ben-Ari.
\end{center}
...
in \emph{Linear Temporal Logic (LTL)}. The formula can be written inline
within the program:
\begin{verbatim}
ltl { [](critical <= 1) }
\end{verbatim}
When loading a file, \eui{} will search for the string \verb+ltl {+, but
always make sure that the box \bu{LTL inline} is checked if you are
performing a verification with an inline formula.

Alternatively, the LTL formula can be stored in a file with the same
name as the source file and extension \p{.prp}. By default, when you
open a \prm{} file, \eui{} opens and loads a \p{.prp} file of the same
name if it exists. The file is saved whenever the source file is used.
...
translation of the LTL formula is displayed. Optionally (configuration
option \p{DISPLAY\_BUCHI}), it can be displayed every time a
verification is run.
